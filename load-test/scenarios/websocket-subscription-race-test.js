 ...
 *   Experiment A uses CROSS_VU_SYNC_BUFFER_MS
...
// KAN-3 fix: Server-side SubscriptionReadinessInterceptor now
// guarantees SUBSCRIBE registration before SEND processing.
// No intra-VU subscription delay is needed.
// Cross-VU sync buffer is still needed for Experiment A to
// ensure both VUs are subscribed before either sends.
        ...
        function (socket) {
            ...
            socket.on('message', function (rawData) {
                ...
                if (stompFrame.command === 'CONNECTED') {
                    ...
                    /*
                     * KAN-3: Server-side SubscriptionReadinessInterceptor
                     * guarantees SUBSCRIBE registration completes before
                     * any subsequent SEND is processed. We send SEND
                     * immediately after SUBSCRIBE with no client-side delay.
                     *
                     * k6 rejects setTimeout(..., 0), so we execute the
                     * SEND path synchronously. The interceptor on the
                     * server ensures ordering.
                     *
                     * Experiment A (sync): additional cross-VU sync buffer
                     * via setTimeout to ensure both VUs have subscribed.
                     * Experiment B (immediate): SEND fires immediately.
                     */
